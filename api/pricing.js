...
async function handler(req, res) {
  try {
    ...
    if (req.method === "POST") {
      ...
      const featuresArr = Array.isArray(features) ? features : (features ?? "").split(",").map(f => f.trim()).filter(Boolean);
    ...
    if (req.method === "PUT") {
      ...
      const body = { ...req.body };
      if (body.features && !Array.isArray(body.features)) body.features = body.features.split(",").map(f => f.trim()).filter(Boolean);
