    ...
    >
      ...
      {plan.icp && (
        <div style={{
          ...
          padding: "10px 12px", marginBottom: plan.limits ? 10 : 0,
      ...
      {plan.limits && (
        <div style={{ color: "var(--text-dim)", fontSize: 11, marginTop: 8, letterSpacing: "-0.01em" }}>{plan.limits}</div>
      )}
...
export default function PricingModule({ plans: plansApi, addons: addonsApi, adminMode }) {
  const { data: plans = [], loading: plansLoading, create: createPlan, remove: removePlan } = plansApi;
  const { data: addons = [], loading: addonsLoading, create: createAddon, remove: removeAddon } = addonsApi;

  const [showPlan,  setShowPlan]  = useState(false);
  const [showAddon, setShowAddon] = useState(false);
  const [saving,    setSaving]    = useState(false);
  const [planForm,  setPlanForm]  = useState({ name:"", price:"", features:"", limits:"", icp:"" });
  const [addonForm, setAddonForm] = useState({ name:"", description:"", price:"", compatiblePlans:"" });

  const savePlan = async () => {
    if (!planForm.name.trim()) return;
    setSaving(true);
    try {
      await createPlan({ ...planForm, features: planForm.features.split(",").map(f => f.trim()).filter(Boolean) });
      setShowPlan(false); setPlanForm({ name:"", price:"", features:"", limits:"", icp:"" });
  ...
  const saveAddon = async () => {
    ...
    try {
      ...
      setShowAddon(false); setAddonForm({ name:"", description:"", price:"", compatiblePlans:"" });
    } catch(e) { alert(e.message); } finally { setSaving(false); }
  };

  // Split DoubleTick vs QuickSell plans
  ...
  const PlanGrid = ({ items, offset = 0 }) => (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 32 }}>
      {items.map((plan, i) => (
        <PlanCard
          key={plan._id || plan.id} plan={plan}
          accent={ACCENT_COLORS[(i + offset) % ACCENT_COLORS.length]}
          adminMode={adminMode}
          onRemove={() => removePlan(plan._id || plan.id)}
        />
      ))}
    </div>
  );
  ...
  return (
    <div className="animate-in">
      ...
      {plansLoading && <p style={{ color:"var(--text-muted)", fontSize:13 }}>Loading…</p>}
      ...
      {plans.length === 0 && !plansLoading && <EmptyState icon="pricing" message="No pricing plans added yet." />}
      ...
      {addonsLoading && <p style={{ color:"var(--text-muted)", fontSize:13 }}>Loading…</p>}
        ...
        : (
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {addons.map((addon, i) => {
              ...
              return (
                <div key={addon._id || addon.id} style={{
                ...
                >
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    {(addon.plans || addon.compatiblePlans) && (
                      <div style={{ color:"var(--text-dim)", fontSize:11, marginTop:4 }}>
                        Compatible: {addon.plans || addon.compatiblePlans}
                      </div>
                    )}
                  </div>
                  ...
                  {adminMode && (
                    <Btn variant="danger" small onClick={() => removeAddon(addon._id || addon.id)}>
      ...
      {/* Conversation Costs info box */}
      <div style={{
        marginTop:24, padding:"16px 20px",
        background:"var(--surface2)", border:"1px solid var(--border2)",
        borderRadius:"var(--radius-md)", borderLeft:"3px solid var(--accent-blue)",
      }}>
        <div style={{ color:"var(--text)", fontWeight:600, fontSize:13.5, marginBottom:8, letterSpacing:"-0.01em" }}>💬 WhatsApp Conversation Costs (INR)</div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
          {[
            ...
            <div key={c.label} style={{ textAlign:"center", padding:"8px 10px", background:"var(--surface)", borderRadius:"var(--radius-sm)", border:"1px solid var(--border)" }}>
              <div style={{ color:"var(--accent-blue)", fontWeight:700, fontSize:15, fontFamily:"var(--font-display)", letterSpacing:"-0.02em" }}>{c.price}</div>
              <div style={{ color:"var(--text-dim)", fontSize:11, marginTop:2, textTransform:"uppercase", letterSpacing:"0.05em" }}>{c.label}</div>
            </div>
          ))}
        </div>
      ...
      {showAddon && (
        <Modal title="Add Add-on" onClose={() => setShowAddon(false)}>
          ...
          <Field label="Compatible Plans" value={addonForm.compatiblePlans} onChange={v => setAddonForm(f=>({...f,compatiblePlans:v}))} placeholder="e.g. Pro, Enterprise" />
          <div style={{ display:"flex", gap:10, marginTop:6 }}>
            <Btn onClick={saveAddon}>{saving ? "Saving…" : "Save Add-on"}</Btn>
